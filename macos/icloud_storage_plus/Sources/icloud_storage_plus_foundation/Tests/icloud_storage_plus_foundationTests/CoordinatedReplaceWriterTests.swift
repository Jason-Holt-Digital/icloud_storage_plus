...
final class CoordinatedReplaceWriterTests: XCTestCase {
    ...
    func testMacOSCopyFailuresReportDestinationRelativePath() throws {
        ...
        XCTAssertEqual(
            ...
            4,
            "copy() should surface the blocked destination path for each "
                + "destination-side failure branch (overwrite, combined "
                + "coordination, combined IO) plus the lane-handler "
                + "defensive fallback."
        )
    }
